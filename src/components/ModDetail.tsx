...
export function ModDetail() {
  ...
  const handleProcessDrafts = async (draftsToProcess: Entry[] = drafts, isAuto = false) => {
    ...
    try {
      ...
      await batch.commit();
      if (!isAuto) {
        alert('Drafts successfully submitted and activity timer has been reset to 7 days.');
      }
    } catch (error: any) {
      console.error('Failed to process drafts to entry', error);
      if (!isAuto) {
        const errorMessage = error.code === 'permission-denied' 
          ? 'Access Denied: You do not have permission to reset this timer or submit these entries.'
          : 'Failed to process drafts. Please check your connection and try again.';
        alert(errorMessage);
      }
    } finally {
  ...
  useEffect(() => {
    if (user && mod && drafts.length > 0) {
      if (mod.role !== 'officer' && Date.now() >= mod.deadlineAt) {
        handleProcessDrafts(drafts, true);
      }
    }
  }, [mod, drafts, user]);
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full inline-flex align-top -ml-1 mt-1 sm:mt-0 pt-2">
                        <div className="mb-3 sm:mb-0 ml-2">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            {isAdmin && assignedMod.phoneNumber && (
