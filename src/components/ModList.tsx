  ...
  ({
    ...
    return (
      <div className="flex flex-col md:flex-row border-t border-white/5 relative bg-black/20">
          ...
          <div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
              <div className="bg-black/50 px-4 py-3 rounded-2xl border border-white/5 shadow-inner flex flex-col gap-1 items-center sm:items-start transition-all hover:bg-zinc-800/40">
                <span className="text-zinc-600 text-[9px] uppercase tracking-[0.2em] font-black">
                  Entries
                </span>
              ...
              {draftsCount > 0 && (
                <div className="bg-indigo-500/10 px-4 py-3 rounded-2xl border border-indigo-500/20 shadow-inner flex flex-col gap-1 items-center sm:items-start animate-pulse">
                  <span className="text-indigo-400 text-[9px] uppercase tracking-[0.2em] font-black">
                    Drafts
                  </span>
                  <strong className="text-xl text-indigo-300 font-black">
                    {draftsCount}
                  </strong>
                </div>
              )}
              ...
              <div className="bg-black/50 px-4 py-3 rounded-2xl border border-white/5 flex flex-col gap-1 items-center sm:items-start shadow-inner transition-all hover:bg-zinc-800/40">
                ...
                <strong className="text-xl text-purple-400 font-black">
                  {mod.entryCount > 0
                    ? ((mod.totalPoints || 0) / mod.entryCount).toFixed(2)
                    : "0.00"}
                </strong>
              </div>
              <div className="bg-black/50 px-4 py-3 rounded-2xl border border-white/5 flex flex-col gap-1 items-center sm:items-start shadow-inner transition-all hover:bg-zinc-800/40">
        ...
        <div
          className={`w-full md:w-[28rem] flex flex-col border-t md:border-t-0 md:border-l border-white/5 relative z-0 ${mod.status === "blacklisted" ? "bg-zinc-900/50 grayscale opacity-75" : mod.role === "officer" ? "bg-zinc-900/40" : isCritical ? "bg-red-950/20" : "bg-black/20"}`}
        >
          <div
            className={`flex-1 flex flex-col p-4 sm:p-6 ${mod.role !== "officer" ? "items-center justify-center" : ""}`}
          >
            <p
              className={`text-[10px] uppercase tracking-[0.3em] font-black mb-4 ${mod.status === "blacklisted" ? "text-zinc-600" : isCritical ? "text-red-500" : "text-zinc-500"} text-center`}
            >
              {mod.role === "officer"
                ? "OFFICER UNITS"
                : mod.status === "blacklisted"
            ...
            <div
              className={`${mod.role !== "officer" ? "flex justify-center w-full" : "flex flex-col gap-3"}`}
            >
              {mod.status === "blacklisted" ? (
                <div className="text-zinc-700 font-mono text-3xl font-black tracking-widest">
                  00:00:00
                </div>
              ) : mod.role === "officer" ? (
                <div className="flex flex-col gap-2 w-full max-h-[250px] overflow-y-auto custom-scrollbar pr-1 text-left">
                  <ol className="space-y-2 text-sm w-full">
                    {relations.map((m) => (
                      <li
                        key={m.id}
                        className="bg-black/40 border border-white/5 rounded-xl p-3 flex items-center justify-between hover:border-blue-500/30 transition-colors group/unit"
                      >
                        <Link
                          to={`/mod/${m.id}`}
                          className="font-black text-white group-hover/unit:text-blue-400 text-base transition-colors truncate max-w-[140px]"
                        >
                          {m.name}
                        </Link>
...
export function ModList() {
  ...
  const [confirmModal, setConfirmModal] = useState<{
    show: boolean;
    modId: string;
    action: "active" | "blacklisted";
  }>({ show: false, modId: "", action: "active" });
  const [showFilterDropdown, setShowFilterDropdown] = useState(false);
  const [modRoleView, setModRoleView] = useState<"moderator" | "officer">(
    "moderator",
  );
  const [newModRole, setNewModRole] = useState<"moderator" | "officer">(
    "moderator",
  );
  const [termsRoleView, setTermsRoleView] = useState<"moderator" | "officer">(
    "moderator",
  );
  ...
  const generateAllEntriesDocx = async () => {
    ...
    try {
      ...
      const allMods = await Promise.all(
        mods.map(async (mod) => {
          const qEntries = query(
            collection(db, "mods", mod.id, "entries"),
            orderBy("createdAt", "desc"),
          );
          const snEntries = await getDocs(qEntries);
          const qDrafts = query(
            collection(db, "mods", mod.id, "drafts"),
            orderBy("createdAt", "desc"),
          );
          const snDrafts = await getDocs(qDrafts);
          return {
            mod,
            entries: snEntries.docs.map((d) => ({
              ...d.data(),
              id: d.id,
            })) as any[],
            drafts: snDrafts.docs.map((d) => ({
              ...d.data(),
              id: d.id,
            })) as any[],
          };
      ...
      allMods
        ...
        .forEach((data, index) => {
          ...
          docContent.push(
            ...
            new Paragraph({
              text: mod.role?.toUpperCase() || "MODERATOR",
              alignment: AlignmentType.CENTER,
              spacing: { after: 200 },
            }),
          );
            ...
            entries.forEach((e, idx) => {
              docContent.push(
                ...
                new Paragraph({
                  children: [
                    new TextRun({
                      text: `   POINTS: +${e.points || 0} | DATE: ${new Date(e.createdAt).toLocaleString()}`,
                      size: 16,
                      color: "71717A",
                    }),
                  ],
              ...
              docContent.push(
                new Paragraph({
                  children: [
                    ...
                    new TextRun({ text: d.text, color: "6366F1" }),
                  ],
                  spacing: { before: 100 },
                }),
                new Paragraph({
                  children: [
                    new TextRun({
                      text: `   DRAFT PTS: +${d.points || 0} | SAVED: ${new Date(d.createdAt).toLocaleString()}`,
                      size: 16,
                      color: "71717A",
                    }),
                  ],
      ...
      saveAs(
        blob,
        `System_Entries_${new Date().toISOString().slice(0, 10)}.docx`,
      );
    } catch (e) {
      console.error(e);
      alert("Failed to generate entries report");
    } finally {
    ...
    try {
      ...
      const modsData = await Promise.all(
        activeMods.map(async (mod) => {
          ...
          const snDrafts = await getDocs(qDrafts);
          const qHonor = query(
            collection(db, "mods", mod.id, "honor_logs"),
            orderBy("createdAt", "desc"),
          );
      ...
      const renderIndexSection = () => {
        const sortedMods = [...modsData].sort(
          (a, b) => (b.mod.totalPoints || 0) - (a.mod.totalPoints || 0),
        );
        return [
          new Paragraph({
            text: "ALL MEMBERS OFFICIAL INDEX",
            heading: HeadingLevel.HEADING_1,
            alignment: AlignmentType.CENTER,
          }),
          new Paragraph({ text: "" }),
                      ...
                      new TableCell({
                        ...
                        children: [
                          new Paragraph({
                            ...
                            children: [
                              new TextRun({
                                text: d.mod.role?.toUpperCase() || "MODERATOR",
                                size: 18,
                              }),
                            ],
                          }),
      ...
      const renderLeaderboardSection = (type: "points" | "pe" | "honor") => {
        ...
        if (type === "points") {
          ...
          sortedMods.sort((a, b) => {
            const peA =
              a.entries.length > 0
                ? (a.mod.totalPoints || 0) / a.entries.length
                : 0;
            const peB =
              b.entries.length > 0
                ? (b.mod.totalPoints || 0) / b.entries.length
                : 0;
            return peB - peA;
          });
        } else if (type === "honor") {
          ...
          headLabel = "HONOR";
          sortedMods.sort(
            (a, b) => (b.mod.honorScore ?? 100) - (a.mod.honorScore ?? 100),
          );
        }
          ...
          new Table({
            ...
            rows: [
              ...
              ...sortedMods.map((d, i) => {
                ...
                if (type === "pe")
                  val = `${d.entries.length > 0 ? ((d.mod.totalPoints || 0) / d.entries.length).toFixed(1) : "0.0"}`;
      ...
      const renderModSection = (data: any) => {
        ...
        const peRatio =
          entries.length > 0
            ? ((mod.totalPoints || 0) / entries.length).toFixed(1)
            : "0.0";
        ...
        const children = [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `UNIQUE ID: ${mod.id?.slice(-12).toUpperCase()}`,
                bold: true,
              }),
            ],
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `GENERATED AT: ${new Date().toLocaleString().toUpperCase()}`,
              }),
            ],
            alignment: AlignmentType.RIGHT,
          }),
          new Paragraph({
            children: [
              new TextRun({
                ...
                bold: true,
                color: status === "BLACKLISTED" ? "DC2626" : "059669",
              }),
            ],
            alignment: AlignmentType.RIGHT,
          }),
        ...
        if (drafts && drafts.length > 0) {
          children.push(
            new Paragraph({
              text: "PENDING PERFORMANCE DRAFTS",
              heading: HeadingLevel.HEADING_3,
            }),
            new Table({
              layout: TableLayoutType.FIXED,
      ...
      const sortedContentMods = [...modsData].sort(
        (a, b) => (b.mod.totalPoints || 0) - (a.mod.totalPoints || 0),
      );
  ...
  const handleAddMod = async (e: React.FormEvent) => {
    ...
    const isDuplicateName = mods.some(
      (mod) =>
        mod.name.toLowerCase() === targetName &&
        (mod.role || "moderator") === newModRole,
    );
    const duplicatePhoneMember = mods.find(
      (mod) => mod.phoneNumber === targetPhone,
    );
    ...
    if (duplicatePhoneMember) {
      ...
      setAddModError(
        `This phone number is already registered to ${duplicatePhoneMember.name} (${role.toUpperCase()}).`,
      );
      return;
    }
  ...
  const handleStatusChange = useCallback(
    async (modId: string, newStatus: "active" | "blacklisted") => {
      if (!isAdmin) return;
  ...
  const roleCounts = useMemo(() => {
    ...
    return {
      moderator: activeMods.filter(
        (m) => (m.role || "moderator") === "moderator",
      ).length,
  ...
  const rankedMods = useMemo(() => {
    ...
    let list = filtered.map((mod) => ({
      ...mod,
      entryCount: entriesMap[mod.id] || 0,
    }));
  ...
  return (
    <div className="flex flex-col h-full bg-black overflow-hidden">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md transition-opacity">
            ...
            >
              <div className="p-6 text-center">
                <div
                  className={`w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center ${confirmModal.action === "blacklisted" ? "bg-red-500/10 text-red-500" : "bg-emerald-500/10 text-emerald-500"}`}
                >
      ...
      {/* Top Header */}
      <header className="h-28 sm:h-32 bg-zinc-900 border-b border-white/5 flex items-center justify-between px-6 sm:px-16 flex-shrink-0 relative z-[60]">
                  ...
                  >
                    ...
                    {isAdmin && (
                      <div className="pt-6 mt-2 border-t border-white/5">
                        ...
                        <button
                          onClick={() => {
                            generateAllEntriesDocx();
                            setShowHeaderMenu(false);
                          }}
                          disabled={isGeneratingPDF}
                          className="bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-400 border border-indigo-500/20 mt-4 px-6 py-5 sm:px-8 sm:py-6 rounded-3xl text-2xl sm:text-3xl font-bold flex items-center gap-5 transition-colors shadow-lg shadow-black/20 w-full disabled:opacity-50"
                        >
                          <Download className="w-10 h-10 sm:w-12 sm:h-12" />
                          <span>
                            {isGeneratingPDF
            ...
            <div className="bg-red-500/10 border border-red-500/20 rounded-3xl p-6 flex items-center gap-4 text-red-500">
              ...
              <div>
                <h3 className="font-bold text-lg">
                  {offline ? "You are offline" : "Connection Error"}
                </h3>
                <p className="text-sm opacity-80">
                  {connectionError || "Restoring connection..."}
                </p>
              </div>
              <button
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-4 p-4 bg-zinc-900 border border-white/5 rounded-[2.5rem] w-full shadow-2xl shadow-black/60 relative z-20">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-black/40 p-2 rounded-3xl border border-white/5 min-w-fit">
              {[
                ...
                >
                  <metric.icon
                    className={`w-4 h-4 ${rankingMetric === metric.id ? metric.color : "text-current"}`}
                  />
            ...
            <div className="flex items-center gap-2 bg-black/40 p-2 rounded-3xl border border-white/5 min-w-fit">
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full ${viewMode === "active" ? "bg-emerald-500 animate-pulse" : "bg-zinc-700"}`}
                />
                Active
              </button>
              <button
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full ${viewMode === "blacklisted" ? "bg-red-500" : "bg-zinc-700"}`}
                />
                Blacklist
              </button>
            </div>
          ...
          <div className="flex items-center justify-center gap-12 border-b border-white/5 pb-2">
            {[
              ...
              >
                <div className="flex items-center gap-3">
                  <span
                    className={`text-sm font-black uppercase tracking-[0.3em] transition-all ${modRoleView === tab.id ? "text-white" : "text-zinc-600 group-hover:text-zinc-400"}`}
                  >
                    {tab.label}
                  </span>
                  <span
                    className={`px-2.5 py-0.5 rounded-lg text-[10px] font-black transition-all ${modRoleView === tab.id ? "bg-blue-600 text-white" : "bg-zinc-800 text-zinc-600"}`}
                  >
                    {tab.count}
                  </span>
                </div>
            ...
            (() => {
              const renderCard = (mod: (typeof mods)[0], index: number) => {
                ...
                const progress =
                  mod.role === "officer"
                    ? 100
                    : Math.max(0, Math.min(100, (timeLeft / totalMs) * 100));
                const rank =
                  sortMode === "ranking" && mod.entryCount > 0
                    ? index + 1
                    : null;
                ...
                return (
                  <motion.div
                    ...
                    className={`group bg-zinc-900 rounded-3xl border border-white/5 shadow-xl shadow-black/40 hover:border-blue-500/20 transition-all duration-300 flex flex-col overflow-hidden relative transform-gpu ${
                      rank === 1
                        ? "ring-2 ring-blue-500/50 ring-offset-4 ring-offset-black"
                        : rank === 2
                          ? "ring-2 ring-zinc-400/30 ring-offset-2 ring-offset-black"
                          : rank === 3
                            ? "ring-2 ring-amber-700/20 ring-offset-2 ring-offset-black"
                            : ""
                    }`}
                  >
                    ...
                    <div
                      ...
                      onClick={() =>
                        setExpandedModId(isExpanded ? null : mod.id)
                      }
                    >
                      <motion.div
                        layout="position"
                      ...
                      <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                        ...
                        <div
                          className={`flex items-center gap-1.5 sm:gap-2 transition-[max-width,opacity,margin] duration-300 ease-out overflow-hidden ${isExpanded ? "max-w-0 opacity-0 !m-0 pointer-events-none" : "max-w-[400px] opacity-100"}`}
                        >
                          <div
                            className="flex items-center gap-1 sm:gap-2 bg-amber-500/10 text-amber-500 px-1.5 py-1 sm:px-3 sm:py-1.5 rounded-md sm:rounded-xl text-[10px] sm:text-sm font-black border border-amber-500/20"
                            title="Points"
                          >
                          ...
                          <div
                            className="flex items-center gap-1 bg-purple-500/10 text-purple-400 px-1.5 py-1 sm:px-3 sm:py-1.5 rounded-md sm:rounded-xl text-[10px] sm:text-sm font-black border border-purple-500/20"
                            title="P/E Ratio"
                          >
                            ...
                            {mod.entryCount > 0
                              ? (
                                  (mod.totalPoints || 0) / mod.entryCount
                                ).toFixed(1)
                              : "0.0"}
                          </div>
                          {mod.role !== "officer" && (
                        ...
                        <div className="pl-1.5 sm:pl-4 border-l border-white/5 flex items-center">
                          <ChevronDown
                            className={`w-5 h-5 sm:w-8 sm:h-8 text-zinc-500 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`}
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    {[...groupSymbols, "Other"].map((key) => {
                      ...
                      return (
                        <div key={key} className="flex flex-col gap-4">
                          <div className="flex items-center gap-4">
                            <span className="text-zinc-500 text-sm font-black uppercase tracking-[0.3em] pl-4 border-l-2 border-zinc-700">
                              Group {key}
                            </span>
                          ...
                          <div className="flex flex-col gap-4">
                            {grouped[key].map((mod) =>
                              renderCard(mod, rankedMods.indexOf(mod)),
                            )}
                          </div>
                        </div>
                      );
                ...
                <form onSubmit={handleAddMod}>
                  <div className="bg-zinc-900 px-6 pb-6 pt-6">
                    ...
                    <div className="mt-2">
                      <label className="block text-sm font-medium text-zinc-300 mb-2">
                        {newModRole === "moderator" ? "Moderator" : "Officer"}{" "}
                        Name
                      </label>
                      <input
                        type="text"
                    ...
                    {newModRole === "officer" && (
                      <div className="mt-4">
                        <label className="block text-sm font-medium text-zinc-300 mb-3">
                          Officer Groups (Select multiple)
                        </label>
                  ...
                  <div className="bg-zinc-800/50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-white/5">
                    ...
                    >
                      {isSubmitting
                        ? "Creating..."
                        : `Create ${newModRole === "moderator" ? "Moderator" : "Officer"}`}
                    </button>
                    <button
                      type="button"
                      onClick={() => setShowAddModal(false)}
